...
setSSID() {
    local wifi=$(/sbin/iw dev | grep Interface | awk '{print $2}')

    local value=""
    if [ \"$wifi\" ]; then
        value=$(/sbin/iw dev $wifi link | grep SSID: | awk '{print $2 $3}')
    fi
...
setUpdates() {
    local icon="$preIcon$postIcon"
    local value=$(dpkg --get-selections | wc -l)
    segmentUpdates="$icon $labelColor Packages $valueColor$value"
}

setDate() {
    local iconDate="$preIcon$postIcon"
    local iconTime="$preIcon$postIcon"
    local date=$(date +'%Y/%m/%d - %A')
    local time=$(date +'%H:%M:%S')

    case $theme in
      'bright-arrow' | 'bright-deco')
         segmentDate="$iconDate ^fg($colGrey500)$date"
         segmentDate+=" $iconTime ^fg($colGrey200)$time, "
      ;;
      'dark-arrow' | 'dark-deco')
         segmentDate="$iconDate ^fg($colGrey200)$date"
         segmentDate+=" $iconTime ^fg($colGrey200)$time, "
      ;;
...
setBattery() {
	local icon="$preIcon$postIcon"
	local value=$(upower -i $(upower -e | grep '/battery') | grep --color=never -E percentage|xargs|cut -d' ' -f2)

	upower -i $(upower -e | grep 'BAT') | grep -E "state|to\ full|percentage"
	segmentBattery="$icon $labelColor Battery $valueColor$value"
}
...
helperNet() {
  local interface=$(/sbin/iw dev | grep Interface | awk '{print $2}')
